...
const searchBoxResult = document.getElementById("searchInfo");

function ckeackValidatioin(){

    let carName = document.getElementById('car-name');
    let selectCar = carName.options[carName.selectedIndex]
    console.log(selectCar.innerText);

    let pickUpLocation = document.getElementById('pick-up');
    let pickUpLocationSelects = pickUpLocation.options[pickUpLocation.selectedIndex]
    console.log(pickUpLocationSelects.innerText);

    let DropLocation = document.getElementById('drop-of');
    let DropLocationSelects = DropLocation.options[DropLocation.selectedIndex]
    console.log(DropLocationSelects.innerText);

    if(drop.value !== ''){
        openSearchResult()
    }
    else{
        alert('Need the drop and pick up date');
    }
}

function openSearchResult() {
    document.getElementsByTagName("main")[0].style.opacity = "0.2";
    searchBoxResult.style.scale = "1";
}
function closeSearchResult() {
  document.getElementsByTagName("main")[0].style.opacity = "1";
  searchBoxResult.style.scale = "0";
}
